...
async function execute(action,value){
  if(action==='open_app'){ const command=allowedApps[String(value||'').toLowerCase()]; if(!command) throw new Error('That application is not in the safe app allowlist.'); launch(command); return 'Opening '+String(value).toLowerCase()+'.' }
  if(action==='open_path'){ const target=allowedPaths[String(value||'').toLowerCase()]; if(!target) throw new Error('That folder is not in the safe path allowlist.'); launch('explorer.exe',[target]); return 'Opening '+String(value).toLowerCase()+'.' }
  if(action==='open_url'){ const url=String(value||'').trim(); if(!/^https?:\/\//i.test(url)) throw new Error('Only http and https URLs are allowed.'); launch('cmd.exe',['/c','start','',url]); return 'Opening the requested website.' }
  if(action==='type_text'){ const t=String(value||''); if(!t||t.length>1000) throw new Error('Text must be between 1 and 1000 characters.'); await powershell(inputType+'\nAdd-Type -AssemblyName System.Windows.Forms\n[System.Windows.Forms.SendKeys]::SendWait(\''+psQuote(t)+'\')'); return 'Typed the requested text.' }
  if(action==='key_press'){ const k=String(value||'').toLowerCase(); const vk=keyCodes[k]; if(!vk) throw new Error('Unsupported key.'); await powershell(inputType+'\n[JarvisInput]::keybd_event('+vk+',0,0,[UIntPtr]::Zero)\n[JarvisInput]::keybd_event('+vk+',0,2,[UIntPtr]::Zero)'); return 'Pressed '+k+'.' }
  if(action==='hotkey'){ const keys=String(value||'').split('+').map(k=>k.trim().toLowerCase()).filter(Boolean); if(keys.length<2||keys.length>4||keys.some(k=>!keyCodes[k])) throw new Error('Hotkey must contain 2-4 supported keys.'); const down=keys.map(k=>'[JarvisInput]::keybd_event('+keyCodes[k]+',0,0,[UIntPtr]::Zero)').join('\n'); const up=[...keys].reverse().map(k=>'[JarvisInput]::keybd_event('+keyCodes[k]+',0,2,[UIntPtr]::Zero)').join('\n'); await powershell(inputType+'\n'+down+'\n'+up); return 'Pressed '+keys.join('+')+'.' }
