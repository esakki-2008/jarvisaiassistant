...
function App() {
  ...
  const sendMessage = useCallback(async (eventOrMessage) => {
    ...
    try {
      ...
      if (mobileCall) {
        if (!mobilePairing?.deviceName) throw new Error('No Android phone is paired. Click PHONE first, then pair your Android phone.')
        setStatus('PHONE CALL')
        ...
        if (routed.tool === 'mobile') {
          if (!mobilePairing?.deviceName) throw new Error('No Android phone is paired. Click PHONE first, then pair your Android phone.')
          if (!routed.action || routed.value === undefined) throw new Error('JARVIS could not determine the phone call.')
          setStatus('PHONE CALL')
  ...
  }, [busy, input, messages, pcOnline, pairing, mobilePairing])
  ...
  const handleStartMobilePairing = async () => {
    ...
    } catch (error) { addAssistantMessage(error.message); setStatus('PHONE PAIRING ERROR') }
    finally { setMobilePairingBusy(false) }
  }

  const handleUnpairMobile = () => { clearMobilePairing(); setMobilePairing(null); setStatus('PHONE UNPAIRED'); setTimeout(() => setStatus('READY'), 1800) }
      ...
      <section className="chat-panel" aria-label="JARVIS command console">
          <header>
            ...
            <div className="header-actions">
              ...
              <button className="memory-button" onClick={handleStartMobilePairing} type="button" disabled={mobilePairingBusy}>{mobilePairing?.deviceName ? 'PHONE PAIRED' : mobilePairing ? 'PAIRING…' : 'PHONE'}</button>
            </div>
          </header>

          {mobilePairing && (
            <div className="memory-strip pairing-strip">
              <div><strong>{mobilePairing.deviceName ? 'PHONE PAIRED' : 'PAIR YOUR ANDROID'}</strong><span>{mobilePairing.deviceName ? mobilePairing.deviceName : 'Enter this code in JARVIS Companion'}</span>{!mobilePairing.deviceName && <><b className="pair-code">{mobilePairing.code}</b><span>Install the Android companion, then enter this code.</span></>}</div>
